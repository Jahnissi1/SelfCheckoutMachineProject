...
public class DisableStationTests {
	...
	public void testDisableActiveSession() {
		session.startSession();
		logic.disableStation();
	}
	
	@Test
	public void testDisableStationFromNormalState() {
		session.stateLogic.gotoState(States.NORMAL);
		logic.disableStation();
		assertEquals("State should now be blocked", States.BLOCKED, session.stateLogic.getState());
	}
	
	@Test
	public void testDisableStationFromBlockedState() {
		session.stateLogic.gotoState(States.BLOCKED);
		logic.disableStation();
		assertEquals("State should still be blocked", States.BLOCKED, session.stateLogic.getState());
	}
}
